...
func TestChecker(t *testing.T) {
	...
	tests := []TestCase{{ast1, ast.IntegerExpression{7}},
		{ast2, ast.BooleanExpression{true}},
		{ast3, ast.IntegerExpression{3}},
		{ast4, ast.BooleanExpression{false}},
		{ast5, ast.IntegerExpression{4}}}
...
func runTests(tests []TestCase, t *testing.T) {
	for i, test := range tests {
		actualValue := Evaluate(test.Node)

		if actualValue == test.ExpectedValue {
			t.Logf("test %d pass, actual type %s, expected type %s: ", i, actualValue, test.ExpectedValue)
			continue
		} else {
			t.Fatalf("test %d fail, actual type %s, expected type %s: ", i, actualValue, test.ExpectedValue)
		}
	}
}
